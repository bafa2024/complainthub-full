...
class ComprehensiveTestSuite {
  ...
  async testRoleBasedAccess() {
    try {
      ...
      if (error.response && error.response.status === 401) {
        await this.addResult('Role-Based Access Control', 'FAILED', 'Token expired or invalid');
        await this.log('❌ Role-based access control failed: Token expired or invalid', 'ERROR');
        return false;
      } else {
  ...
  async generateReport() {
    ...
    await this.log('='.repeat(60), 'REPORT');
