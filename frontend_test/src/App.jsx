import React, { useState, useEffect } from 'react';
import axios from 'axios';

const API_BASE_URL = 'http://localhost:8000';

function App() {
  const [backendStatus, setBackendStatus] = useState(null);
  const [testResults, setTestResults] = useState({});
  const [loading, setLoading] = useState(false);

  const testBackendHealth = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/health`);
      setBackendStatus(response.data);
    } catch (error) {
      setBackendStatus({ status: 'error', message: error.message });
    }
    setLoading(false);
  };

  const testEndpoints = async () => {
    setLoading(true);
    const endpoints = [
      { name: 'Root', url: '/' },
      { name: 'Health Check', url: '/health' },
      { name: 'Test Endpoint', url: '/test' },
      { name: 'Testing Dashboard', url: '/api/v1/testing/' },
      { name: 'Database Test', url: '/api/v1/testing/database' },
      { name: 'CRUD Test', url: '/api/v1/testing/crud' },
      { name: 'Mock Data Test', url: '/api/v1/testing/mock-data' }
    ];

    const results = {};
    
    for (const endpoint of endpoints) {
      try {
        const response = await axios.get(`${API_BASE_URL}${endpoint.url}`);
        results[endpoint.name] = {
          status: 'success',
          statusCode: response.status,
          data: response.data
        };
      } catch (error) {
        results[endpoint.name] = {
          status: 'error',
          statusCode: error.response?.status || 'N/A',
          message: error.message
        };
      }
    }
    
    setTestResults(results);
    setLoading(false);
  };

  useEffect(() => {
    testBackendHealth();
  }, []);

  return (
    <div className="container-fluid p-4">
      <div className="row">
        <div className="col-12">
          <h1 className="mb-4 text-center">🔗 Frontend-Backend Integration Test</h1>
          
          <div className="alert alert-info">
            <h5>Integration Test Status</h5>
            <p className="mb-0">This interface tests the connection between the React frontend and FastAPI backend.</p>
          </div>

          {/* Backend Status Card */}
          <div className="card mb-4">
            <div className="card-header bg-primary text-white">
              <h3 className="mb-0">🖥️ Backend Status</h3>
            </div>
            <div className="card-body">
              {loading && (
                <div className="d-flex align-items-center">
                  <div className="spinner-border me-2" role="status"></div>
                  <span>Testing backend connection...</span>
                </div>
              )}
              
              {backendStatus && !loading && (
                <div>
                  <div className={`alert ${backendStatus.status === 'healthy' ? 'alert-success' : 'alert-danger'}`}>
                    <h5 className="alert-heading">
                      {backendStatus.status === 'healthy' ? '✅ Backend Healthy' : '❌ Backend Error'}
                    </h5>
                    <strong>Status:</strong> {backendStatus.status}
                    {backendStatus.message && (
                      <div><strong>Message:</strong> {backendStatus.message}</div>
                    )}
                    {backendStatus.timestamp && (
                      <div><small>Last checked: {new Date(backendStatus.timestamp).toLocaleString()}</small></div>
                    )}
                  </div>
                  
                  {backendStatus.status === 'healthy' && (
                    <div className="row">
                      <div className="col-md-4">
                        <strong>API Version:</strong> {backendStatus.api_version}
                      </div>
                      <div className="col-md-4">
                        <strong>Database:</strong> 
                        <span className={`ms-1 badge ${backendStatus.database === 'success' ? 'bg-success' : 'bg-danger'}`}>
                          {backendStatus.database}
                        </span>
                      </div>
                      <div className="col-md-4">
                        <strong>Backend URL:</strong> {API_BASE_URL}
                      </div>
                    </div>
                  )}
                </div>
              )}
              
              <div className="mt-3">
                <button 
                  className="btn btn-primary me-2" 
                  onClick={testBackendHealth}
                  disabled={loading}
                >
                  🔄 Refresh Status
                </button>
                <button 
                  className="btn btn-success" 
                  onClick={testEndpoints}
                  disabled={loading}
                >
                  🧪 Test All Endpoints
                </button>
              </div>
            </div>
          </div>

          {/* Test Results */}
          {Object.keys(testResults).length > 0 && (
            <div className="card">
              <div className="card-header bg-success text-white">
                <h3 className="mb-0">🧪 Endpoint Test Results</h3>
              </div>
              <div className="card-body">
                <div className="row">
                  {Object.entries(testResults).map(([name, result]) => (
                    <div key={name} className="col-md-6 col-lg-4 mb-3">
                      <div className={`card h-100 border-${result.status === 'success' ? 'success' : 'danger'}`}>
                        <div className="card-header d-flex justify-content-between align-items-center">
                          <strong>{name}</strong>
                          <span className={`badge ${result.status === 'success' ? 'bg-success' : 'bg-danger'}`}>
                            {result.statusCode}
                          </span>
                        </div>
                        <div className="card-body">
                          {result.status === 'success' ? (
                            <div>
                              <div className="text-success mb-2">✅ Success</div>
                              {result.data && (
                                <details>
                                  <summary className="btn btn-sm btn-outline-info">View Response</summary>
                                  <pre className="mt-2 small bg-light p-2 rounded">
                                    {JSON.stringify(result.data, null, 2)}
                                  </pre>
                                </details>
                              )}
                            </div>
                          ) : (
                            <div>
                              <div className="text-danger mb-2">❌ Failed</div>
                              <small className="text-muted">{result.message}</small>
                            </div>
                          )}
                        </div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            </div>
          )}

          {/* Quick Links */}
          <div className="card mt-4">
            <div className="card-header bg-info text-white">
              <h3 className="mb-0">🔗 Quick Links</h3>
            </div>
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  <h5>Backend URLs:</h5>
                  <ul className="list-group list-group-flush">
                    <li className="list-group-item">
                      <a href="http://localhost:8000" target="_blank" rel="noopener noreferrer">
                        Backend Root 🏠
                      </a>
                    </li>
                    <li className="list-group-item">
                      <a href="http://localhost:8000/docs" target="_blank" rel="noopener noreferrer">
                        API Documentation 📚
                      </a>
                    </li>
                    <li className="list-group-item">
                      <a href="http://localhost:8000/health" target="_blank" rel="noopener noreferrer">
                        Health Check ❤️
                      </a>
                    </li>
                    <li className="list-group-item">
                      <a href="http://localhost:8000/api/v1/testing/" target="_blank" rel="noopener noreferrer">
                        Testing Dashboard 🧪
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="col-md-6">
                  <h5>Environment Info:</h5>
                  <ul className="list-group list-group-flush">
                    <li className="list-group-item">
                      <strong>Frontend URL:</strong> {window.location.origin}
                    </li>
                    <li className="list-group-item">
                      <strong>React Version:</strong> {React.version}
                    </li>
                    <li className="list-group-item">
                      <strong>Environment:</strong> Development
                    </li>
                    <li className="list-group-item">
                      <strong>CORS:</strong> Enabled
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>

          {/* Integration Status Summary */}
          <div className="alert alert-light mt-4">
            <h5>🔧 Integration Summary</h5>
            <div className="row">
              <div className="col-md-4">
                <strong>Backend:</strong> ✅ Running (Port 8000)
              </div>
              <div className="col-md-4">
                <strong>Frontend:</strong> ✅ Running (Port 3000)
              </div>
              <div className="col-md-4">
                <strong>Communication:</strong> ✅ CORS Enabled
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default App;